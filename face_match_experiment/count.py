...
def write_csv(root_name,truth_label,count,lag,window):
	l=[]
	file_name="data/count_diff_l_"+str(lag)+"_w_"+str(window)+".csv"
...
def min_distance_index(feature,b_feature):
	min_index=0
	min_distance=compute_euclidian_distance(feature,b_feature[0])
	for i in range(1,len(b_feature)):
		distance=compute_euclidian_distance(feature,b_feature[i])
...
for r_features in relevant_features:
	...
	for feature in r_features:
		index=min_distance_index(feature,b_feature)
		c[index]+=1
